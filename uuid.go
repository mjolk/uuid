...
import (
	...
	"github.com/mjolk/uint128"
)

// UUID is a thin wrapper around "github.com/gofrs/uuid".UUID that can be
...
func Equal(u1 UUID, u2 UUID) bool {
	return bytes.Equal(u1.UUID[:], u2.UUID[:])
}
...
// Bytes shadows (*github.com/gofrs/uuid.UUID).Bytes() to prevent UUID
...
// MakeV4 delegates to "github.com/gofrs/uuid".NewV4 and wraps the result in
// a UUID.
func NewV4() UUID {
...
func NewV1() UUID {
	return UUID{uuid.Must(uuid.NewV1())}
}
...
// FromBytes delegates to "github.com/gofrs/uuid".FromBytes and wraps the
// result in a UUID.
...
// FromString delegates to "github.com/gofrs/uuid".FromString and wraps the
// result in a UUID.
...
// FromUint128 delegates to "github.com/gofrs/uuid".FromBytes and wraps the
// result in a UUID.
func FromUint128(input uint128.Uint128) UUID {
	u, err := uuid.FromBytes(input.GetBytes())
	if err != nil {
		panic(fmt.Errorf("should never happen with 16 byte slice"))
	}
	return UUID{u}
}
